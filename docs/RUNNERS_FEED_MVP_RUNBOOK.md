...
오른쪽 위 `단계 데이터 상태` panel은 현재 단계를 표시하는 panel이 아니라 선택한
Job의 계측 가능 여부를 나타낸다. 숫자 상태값을 Grafana value mapping으로 다음과
같이 표시한다.
...
실시간 현재 단계와 경과시간은 `선택한 Job의 9단계 처리시간`과
`단계별 상태와 시각` panel에서 확인한다.
