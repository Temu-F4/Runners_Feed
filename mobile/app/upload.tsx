...
function videoFromAsset(asset: ImagePickerAsset): SelectedVideo {
  ...
  const supported = (lower.endsWith(".mp4") && mimeType === "video/mp4")
    || (lower.endsWith(".mov") && mimeType === "video/quicktime");
  if (!supported) validation = "invalid_type";
...
function validationMessage(validation: UploadValidation) {
  if (validation === "invalid_type") return "MP4 또는 MOV 영상만 선택할 수 있습니다. 다른 파일을 선택해 주세요.";
...
export default function UploadScreen() {
  ...
  return (
    <Screen>
      ...
      <Pressable accessibilityLabel="촬영 가이드" accessibilityRole="button" onPress={() => setGuideOpen((open) => !open)} style={{ backgroundColor: colors.surface, borderColor: colors.border, borderWidth: 1, flexDirection: "row", gap: 12, minHeight: 116, padding: 10 }}>
        ...
        <View style={{ flex: 1, justifyContent: "center" }}><Text style={styles.eyebrow}>촬영 가이드</Text><Text style={{ color: colors.primary, fontSize: 13, fontWeight: "800", lineHeight: 19, marginTop: 8 }}>측면 · 전신{`\n`}3–10초</Text><Text style={[styles.caption, { marginTop: 8 }]}>{guideOpen ? "안내 닫기 ↑" : "안내 자세히 보기 →"}</Text></View>
      </Pressable>
      {guideOpen ? <View style={{ backgroundColor: colors.surfaceSecondary, borderColor: colors.border, borderTopWidth: 0, borderWidth: 1, padding: 12 }}><Text style={styles.body}>카메라를 허리 높이에 고정하고, 달리는 사람의 머리부터 발끝까지 옆모습이 계속 보이게 촬영하세요.</Text><Text style={[styles.caption, { marginTop: 7 }]}>밝은 장소에서 한 사람만 담고, 카메라를 움직이거나 확대하지 마세요. 현재는 정적 촬영 안내이며 샘플 영상은 제공하지 않습니다.</Text></View> : null}
