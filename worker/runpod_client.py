...
class RunPodVideoAnalysisClient:
    def __init__(self, endpoint: str, token: str, *, timeout: float = 3700, opener=urlopen) -> None:
        ...
        self.endpoint = (
            base
            if base.endswith("/v4/storage-video-analysis")
            else base + "/v4/storage-video-analysis"
        )
        self.token = token
    ...
    def submit(self, payload: dict) -> dict:
        ...
        try:
            ...
            raise RuntimeError(f"RunPod rejected request with HTTP {error.code}") from error
